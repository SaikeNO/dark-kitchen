...
public sealed partial class ProjectReferenceArchitectureTests
{
    ...
    [Fact]
    public void EndpointHandlers_HaveMatchingIntegrationTests()
    {
        var root = RepositoryPaths.FindRoot();
        ...
        foreach (var contextDirectory in GetServiceContextDirectories(root))
        {
            ...
            var featuresDirectory = Path.Combine(contextDirectory, $"DarkKitchen.{contextName}.Features", "Features");
            var integrationTestsDirectory = Path.Combine(
                root,
                "tests",
                "Integration",
                contextName,
                $"DarkKitchen.{contextName}.IntegrationTests");

            var endpointFiles = Directory
                .EnumerateFiles(featuresDirectory, "*Endpoint.cs", SearchOption.AllDirectories)
            ...
            Assert.True(
                Directory.Exists(integrationTestsDirectory),
                $"{contextName} endpoint handlers require an integration test project at {integrationTestsDirectory}.");

            var integrationTestFiles = Directory
                .EnumerateFiles(integrationTestsDirectory, "*EndpointTests.cs", SearchOption.AllDirectories)
                .Select(path => Path.GetFileName(path))
                .ToHashSet(StringComparer.OrdinalIgnoreCase);

            foreach (var endpointFile in endpointFiles)
            {
                var expectedTestFileName = $"{Path.GetFileNameWithoutExtension(endpointFile)}Tests.cs";
                Assert.Contains(expectedTestFileName, integrationTestFiles);
            }
        }
    }
