...
public static class UpsertRecipeEndpoint
{
        ...
        CancellationToken ct)
    {
        ...
        {
            existingRecipe = new Recipe { ProductId = productId };
            db.Recipes.Add(existingRecipe);
        }

        db.RecipeItems.RemoveRange(existingRecipe.Items);
        existingRecipe.Items = request.Items
            .Select(item => new RecipeItem
            {
                ProductId = productId,
            ...
            .ToList();
        existingRecipe.UpdatedAt = DateTimeOffset.UtcNow;
