  ...
  (error: unknown) => {
    if (axios.isAxiosError(error)) {
      ...
      const fallback = status === undefined
        ? "Nie mozna polaczyc sie z API."
        : `Zadanie zakonczylo sie bledem ${status}.`;

      return Promise.reject(new ApiRequestError(problemMessage(problem, fallback), status));
    }

    return Promise.reject(error instanceof Error ? error : new Error("Nieznany blad API."));
  }
);
...
export async function patchJson<TResponse>(url: string, data: unknown) {
  const response = await http.patch<TResponse>(url, data);
  return response.data;
}

export function errorMessage(error: unknown, fallback = "Operacja nie powiodla sie.") {
